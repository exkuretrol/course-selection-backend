...
def ner_query(text: List[NerToken]) -> str:
    subject = []  # 課程
    time_lesson = []  # 時間_節
    time_week = []  # 時間_週
    people = []  # 老師
    ...
    for sentence in text:
        ...
        if 'SUBJECT' in sentence.ner:  # 科目
            ...
            se = pinyin(word, style=Style.BOPOMOFO)  # 注音
            se = "%%".join(
                [item for innerlist in se for item in innerlist])  # 刪[]
            ...
            try:
                ...
                # raise RuntimeError(f"key ${e} not found")
                print(f"\nkey ${e} not found\n")
    ...
    res = dict(people=people, category=category, grade=grade, department=department, time_lesson=time_lesson, time_week=time_week, campus=campus,
               subject=subject, credit=credit, semester=semester, education=education, internship=internship, graduate=graduate, stuclass=Class)
    print(res)
    ...
    for k, v in res.items():
        ...
        if k == 'time_lesson' and len(v) > 0:
            for search in v:
                l = [
                    f"`上課日期／節次` like '%%{search[i:i+2]}%%'" for i in range(0, len(search), 2)]
